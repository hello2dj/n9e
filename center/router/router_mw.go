...
func (rt *Router) handleCustomAuth(c *gin.Context) (res *models.User, err error) {
	ret, err := rt.CustomAuth(c)
	if err != nil {
		return nil, err
	}
...
func (rt *Router) jwtAuth() gin.HandlerFunc {
	return func(c *gin.Context) {
		userid, username, err := rt.handleJWTToken(c)
		if err != nil && rt.CustomAuth == nil {
			ginx.Bomb(http.StatusUnauthorized, "unauthorized")
		}

		if err != nil && rt.CustomAuth != nil {
			user, err := rt.handleCustomAuth(c)
			if err != nil {
				ginx.Bomb(http.StatusUnauthorized, "unauthorized")
			}
			userid = user.Id
...
func (rt *Router) handleJWTToken(c *gin.Context) (userid int64, username string, err error) {
