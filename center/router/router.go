...
import (
	...
	"cncamp/pkg/third_party/nightingale/center/cstats"
...
type Router struct {
	...
	Redis             storage.Redis
	MetaSet           *metas.Set
	CustomAuth        CustomAtuhHandler
	CheckPerm         CheckPermHandler
	BusiGroupChecker  BusiGroupChecker
	TargetCache       *memsto.TargetCacheType
	Sso               *sso.SsoClient
	Ctx               *ctx.Context
}

type CheckPermHandler = func(ctx *gin.Context, me *models.User, opration string) (can bool, ok bool)
type BusiGroupChecker = func(ctx *gin.Context, me *models.User, bg *models.BusiGroup) (can bool, err error)
type CustomAtuhHandler = func(ctx *gin.Context) (*models.User, error)
...
func New(httpConfig httpx.Config, center cconf.Center, operations cconf.Operation, ds *memsto.DatasourceCacheType, ncc *memsto.NotifyConfigCacheType,
	pc *prom.PromClientMap, redis storage.Redis, sso *sso.SsoClient, ctx *ctx.Context, metaSet *metas.Set, tc *memsto.TargetCacheType, customAuth CustomAtuhHandler, checkPermHandler CheckPermHandler, bgChecker BusiGroupChecker) *Router {
...
func (rt *Router) frontendRoute(r *gin.Engine) {
	r.NoRoute(func(c *gin.Context) {
		if !strings.Contains(c.Request.URL.Path, "/api/n9e") {
			c.Next()
			return
		}
		reqPath := strings.TrimPrefix(c.Request.URL.Path, "/api/n9e/")
		arr := strings.Split(reqPath, ".")
		suffix := arr[len(arr)-1]
		switch suffix {
		case "png", "jpeg", "jpg", "svg", "ico", "gif", "css", "js", "html", "htm", "gz", "zip", "map":
			c.File(path.Join(strings.Split("pub/"+reqPath, "/")...))
		default:
